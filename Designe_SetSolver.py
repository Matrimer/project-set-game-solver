...
#### LEFT SIDE       ####
#### Grid of cads    ####
        # Define cardGridWidget
        self.cardGridWidget = QtWidgets.QWidget(self.centralwidget)
        self.cardGridWidget.setGeometry(QtCore.QRect(10, 10, 431, 381))
        self.cardGridWidget.setObjectName("cardGridWidget")
        # Create cardGrid
        self.cardGrid = QtWidgets.QGridLayout(self.cardGridWidget)
        self.cardGrid.setContentsMargins(0, 0, 0, 0)
        self.cardGrid.setObjectName("cardGrid")

        # Create and draw cardGridBox_n at position (x,y) in the cardGrid
        def newCardGridBox(n, x, y) :
            self.cardGridBox = QtWidgets.QGraphicsView(self.cardGridWidget)
            self.cardGridBox.setObjectName("cardGridBox_" + str(n))
            self.cardGridBox.setAutoFillBackground(True)
            self.cardGrid.addWidget(self.cardGridBox, y, x, 1, 1)

        # Fill cardGrid with 3x4 cardGridBoxes
        def createCardGridBoxes() :
            i = 0
            for x in range(0,3) :
                for y in range (0,4) :
                    newCardGridBox(i, x, y)
                    i = i + 1

        createCardGridBoxes() # Draw boxes in the cardGrid
